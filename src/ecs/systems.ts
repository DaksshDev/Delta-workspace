...
export const BackupSystem = {
  ...
  async importAll(jsonStr: string) {
    ...
    const { getDB } = await import('./store');
    const db = await getDB();
    const tx = db.transaction(['entities', 'components', 'studyItems', 'tags', 'bucket-tags', 'idea-tags', 'folders', 'settings', 'bugs', 'syncQueue'], 'readwrite');
    ...
    for (const e of asArray(data.entities)) await tx.objectStore('entities').put(e);
    for (const c of asArray(data.components)) await tx.objectStore('components').put(c);
    for (const item of asArray(data.studyItems)) await tx.objectStore('studyItems').put(item);
    for (const tag of asArray(data.tags)) await tx.objectStore('tags').put(tag);
    for (const tag of asArray(data.bucketTags)) await tx.objectStore('bucket-tags').put(tag);
    for (const tag of asArray(data.ideaTags)) await tx.objectStore('idea-tags').put(tag);
    for (const folder of asArray(data.folders)) await tx.objectStore('folders').put(folder);
    for (const setting of asArray(data.settings)) await tx.objectStore('settings').put(setting);
    for (const bug of asArray(data.bugs)) await tx.objectStore('bugs').put(bug);
    await tx.done;
  }
}
